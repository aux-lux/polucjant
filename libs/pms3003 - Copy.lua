...
-- unpack message
  ...
  for n=-1,dlen do
    ...
    if M.debug==true then
      --print2(('  data#%2d: 0x%02X%02X=%6d cksum:%6d'): format(n,msb,lsb,pms[n],cksum))
    end
  end
...
-- particulate mater (PM)
  ...
  if M.verbose==true then
    --print2(('%s: %4s,%4s,%4s [ug/m3]') :format(M.name,M.pm01 or 'null',M.pm25 or 'null',M.pm10 or 'null'))
  end
-- particle (number) size distribution (PSD)
  ...
  if M.verbose==true and M.psd then
    --print2(('%s: %4s,%4s,%4s,%4s,%4s [#/100cm3]') :format(M.name,unpack(psd)))
  end
end
...
-- initialization
  if type(pinSET)=='number' then
    ...
    if M.verbose==true then
      -- print2(('%s: data acquisition %s.\n  Console %s.') :format(M.name,type(status)=='string' and status or 'paused','enhabled'))
    end
...
-- start sampling: continuous sampling mode
  if M.verbose==true then
    --print2(('%s: data acquisition %s.\n  Console %s.') :format(M.name,'started','dishabled'))
  end
